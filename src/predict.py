...
def main():
    ...
    parser.add_argument("-v", "--verbose", action="store_true", help="show the image, and the prediction time.")
    args = parser.parse_args()


    model = tf.keras.models.load_model(args.model, compile=False)
    img = cv.imread(args.input, cv.IMREAD_GRAYSCALE)
    img_a = utils.process_image_prediction(img, verbose=args.verbose)
